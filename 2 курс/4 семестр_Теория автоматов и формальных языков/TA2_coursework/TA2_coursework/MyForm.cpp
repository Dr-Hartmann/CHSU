...
bool TA2coursework::MyForm::allowed_character(wchar_t* c)
{
	return (*c == '_' || *c == 'a' || *c == 'b' || *c == 'c' || *c == 'd' || *c == 'e' || *c == 'f'
		...
		|| *c == 'W' || *c == 'X' || *c == 'Y' || *c == 'Z' || numbers(c) || *c == '0') ? true : false;
}
	...
	if (str != "") {
		for (int i = 0; i < str->Length; i++) {
			...
			if (S == 0 && word != "")//если нет этого слова в других категориях
				...
				error(&word, "clstun", &line);

			if (S == 6 || S == 135 || S == 139 || S == 152 || S == 153 || S == 156 || S == 173) i--;//предпросмотр для занесения слова без чтения символа
